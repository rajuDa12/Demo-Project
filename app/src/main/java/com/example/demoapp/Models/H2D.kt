...
object H2D {
    ...
    fun getOutput3(str: String): String {
        ...
        str1List.reverse()
            for ((index, str) in str1List.withIndex()) {
                ans += str * 16.0.pow(index.toDouble())
                var vali = str * 16.0.pow(index.toDouble())


                if (index < str1List.size - 1) {
                    sumval1 += "$vali+"
                    value += "($str x 16^$index)+"
                } else {
                    sumval1 += "$vali"
                    value += "($str x 16^$index)"
                }
            ...
            for ((index, str2) in str2List.withIndex()) {
                ...
                ans2 += str2 * 16.0.pow(-index.toDouble().plus(1.0))
                value2 += "+($str2 x 16^${-index.plus(1)})"
                var valid = str2 * 16.0.pow(-index.toDouble().plus(1.0))
                sumval2 += "+$valid"
